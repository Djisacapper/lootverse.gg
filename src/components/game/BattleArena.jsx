...
:root {
  --gold:#f5c842; --gold-dim:rgba(245,200,66,.15); --gold-glow:rgba(245,200,66,.4);
  --purple:#9d6fff; --purple-dim:rgba(157,111,255,.12);
  --bg-deep:#03000d; --bg-card:#0a0718;
  --border-subtle:rgba(255,255,255,.06); --border-gold:rgba(245,200,66,.22);
  --text-bright:#f0eaff; --text-mid:rgba(240,234,255,.5); --text-dim:rgba(240,234,255,.25);
  --green:#00e5a0; --cyan:#00e5ff;
}
.ba{font-family:'Outfit',sans-serif;color:var(--text-bright);}
.ba-title{font-family:'Rajdhani',sans-serif;letter-spacing:.06em;}
.ba-noise{position:absolute;inset:0;pointer-events:none;z-index:1;background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.035'/%3E%3C/svg%3E");background-repeat:repeat;background-size:150px;mix-blend-mode:overlay;}
@keyframes ba-scan{0%{top:-2px;opacity:0}4%{opacity:1}96%{opacity:.5}100%{top:100%;opacity:0}}
.ba-scan{position:absolute;left:0;right:0;height:1px;z-index:10;pointer-events:none;background:linear-gradient(90deg,transparent 0%,var(--gold) 30%,var(--gold) 70%,transparent 100%);opacity:0;animation:ba-scan 8s linear infinite;}
@keyframes ba-rise{0%{transform:translateY(0) translateX(0) scale(1);opacity:0}10%{opacity:1}85%{opacity:.4}100%{transform:translateY(-100px) translateX(var(--dx)) scale(0);opacity:0}}
...
@keyframes ba-winner-pulse{0%,100%{box-shadow:0 0 0 1.5px rgba(245,200,66,.3),0 0 35px rgba(245,200,66,.1),inset 0 0 30px rgba(245,200,66,.04)}50%{box-shadow:0 0 0 2px rgba(245,200,66,.55),0 0 65px rgba(245,200,66,.2),inset 0 0 45px rgba(245,200,66,.09)}}
...
@keyframes ba-item-in{0%{transform:translateX(-14px) scale(.94);opacity:0}100%{transform:translateX(0) scale(1);opacity:1}}
.ba-item-in{animation:ba-item-in .3s cubic-bezier(.22,1,.36,1) forwards;}
.ba-spin-slot{flex-shrink:0;height:252px;position:relative;border-radius:12px;overflow:hidden;background:#04010e;border:1px solid rgba(157,111,255,.18);}
@keyframes ba-gem-float{0%,100%{transform:translateY(0) scale(1) rotate(-4deg);filter:drop-shadow(0 0 12px rgba(167,139,250,.9))}50%{transform:translateY(-5px) scale(1.08) rotate(4deg);filter:drop-shadow(0 0 28px rgba(167,139,250,1)) drop-shadow(0 0 8px rgba(245,200,66,.6))}}
@keyframes ba-gem-reveal{0%{transform:scale(0.2) rotate(-30deg);opacity:0}60%{transform:scale(1.18) rotate(6deg);opacity:1}100%{transform:scale(1) rotate(0deg);opacity:1}}
@keyframes ba-gem-ring{0%{transform:scale(1);opacity:.7;border-color:rgba(167,139,250,.8)}100%{transform:scale(2.2);opacity:0;border-color:rgba(245,200,66,.3)}}
.ba-gem-float{animation:ba-gem-float 1.6s ease-in-out infinite;}
.ba-gem-reveal{animation:ba-gem-reveal .55s cubic-bezier(.34,1.56,.64,1) forwards;}
.ba-gem-ring-anim{animation:ba-gem-ring 1.4s ease-out infinite;}
...
@keyframes ba-cdpop{0%{transform:scale(0.04) rotate(-10deg);opacity:0}65%{transform:scale(1.1) rotate(2deg);opacity:1}100%{transform:scale(1) rotate(0deg);opacity:1}}
.ba-cdnum{animation:ba-cdpop .48s cubic-bezier(.34,1.56,.64,1) forwards;}
@keyframes ba-win-in{0%{transform:translateY(-22px) scale(.95);opacity:0}100%{transform:translateY(0) scale(1);opacity:1}}
...
.ba-col{position:relative;flex:1;display:flex;flex-direction:column;border-radius:18px;overflow:hidden;background:var(--bg-card);border:1.5px solid var(--border-subtle);min-width:0;transition:border-color .35s,box-shadow .35s;}
.ba-btn-ghost{background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.09);border-radius:10px;color:var(--text-mid);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .18s,border-color .18s,color .18s;font-family:'Outfit',sans-serif;}
.ba-btn-ghost:hover{background:rgba(245,200,66,.1);border-color:rgba(245,200,66,.3);color:var(--gold);}
.ba-btn-gold{background:linear-gradient(135deg,#f5c842 0%,#e8a800 100%);border:none;border-radius:12px;color:#0a0600;font-weight:900;font-family:'Outfit',sans-serif;cursor:pointer;box-shadow:0 0 28px rgba(245,200,66,.3),0 4px 16px rgba(0,0,0,.4);transition:transform .18s,box-shadow .18s;}
...
const startKeepalive=(c)=>{if(_keepaliveStarted)return;_keepaliveStarted=true;try{const osc=c.createOscillator(),gain=c.createGain();gain.gain.value=0;osc.connect(gain);gain.connect(c.destination);osc.start();}catch{}};
const unlockAudio=()=>{const c=getCtx();if(!c)return;if(c.state==='suspended')c.resume();try{const b=c.createBuffer(1,1,c.sampleRate),s=c.createBufferSource();s.buffer=b;s.connect(c.destination);s.start(0);}catch{}startKeepalive(c);};
...
const playSpin=(fast)=>{stopSpin();const dur=fast?1500:3100,t0=Date.now();const tick=()=>{const c=getCtx();if(c&&c.state==='running'){try{const n=c.currentTime,o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);o.type='triangle';o.frequency.setValueAtTime(260+Math.random()*120,n);g.gain.setValueAtTime(0.028,n);g.gain.exponentialRampToValueAtTime(0.001,n+.065);o.start(n);o.stop(n+.065);}catch{}}const el=Date.now()-t0;if(el<dur){const p=el/dur;_tick=setTimeout(tick,35+p*290);}};const c=getCtx();if(c&&c.state==='suspended'){c.resume().then(tick).catch(tick);}else tick();};
const playReward=()=>{const c=getCtx();if(!c)return;const run=()=>{try{const n=c.currentTime;[[523.25,0],[659.25,.11],[783.99,.22],[1046.5,.34]].forEach(([freq,dt])=>{const o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);o.type='sine';o.frequency.setValueAtTime(freq,n+dt);g.gain.setValueAtTime(0,n+dt);g.gain.linearRampToValueAtTime(0.16,n+dt+.04);g.gain.exponentialRampToValueAtTime(0.001,n+dt+.48);o.start(n+dt);o.stop(n+dt+.5);});[523.25,659.25,783.99].forEach(freq=>{const o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);o.type='triangle';o.frequency.setValueAtTime(freq,n+.34);g.gain.setValueAtTime(0,n+.34);g.gain.linearRampToValueAtTime(0.055,n+.46);g.gain.exponentialRampToValueAtTime(0.001,n+1.4);o.start(n+.34);o.stop(n+1.4);});}catch{}};if(c.state==='suspended'){c.resume().then(run).catch(run);}else run();};

const Particles=React.memo(({accent='#f5c842',count=10})=>{const pts=useRef(Array.from({length:count},(_,i)=>({id:i,left:`${6+Math.random()*88}%`,bottom:`${Math.random()*14}%`,size:1.2+Math.random()*2.2,d:`${3+Math.random()*5}s`,delay:`${-Math.random()*7}s`,dx:`${(Math.random()-.5)*34}px`}))).current;return <div style={{position:'absolute',inset:0,pointerEvents:'none',overflow:'hidden'}}>{pts.map(p=><div key={p.id} className="ba-pt" style={{left:p.left,bottom:p.bottom,width:p.size,height:p.size,background:accent,boxShadow:`0 0 ${p.size*3}px ${accent}`,'--d':p.d,'--delay':p.delay,'--dx':p.dx}}/>)}</div>;});
    ...
    <div style={{
      ...
      {[0,1].map(i=>(
        <div key={i} className="ba-gem-ring-anim" style={{
          position:'absolute',width:50+i*20,height:50+i*20,borderRadius:'50%',
      ...
      <span style={{
        fontSize:10,fontWeight:900,color:'#c4b5fd',letterSpacing:'.22em',
        textTransform:'uppercase',position:'relative',zIndex:2,
        textShadow:'0 0 16px rgba(167,139,250,.9)',
      }}>GEM SPIN</span>
      <span style={{fontSize:16,position:'relative',zIndex:2}}>💎</span>
    </div>
  );
...
const PlayerColumn=({player,playerColor:pc,isWinner,wonItems,spinPhase,caseItems,spinnerKey,spinnerItem,magicItem,onSpinDone,onGemSpinDone,fast,showPct,pct})=>{
  ...
  const activeItem=spinPhase==='gem_spin'?resolvedMagicItem:spinnerItem;
  ...
  return(
    <div className={`ba-col${isWinner?' ba-winner':''}`} style={{border:`1.5px solid ${isWinner?'rgba(245,200,66,.35)':pc+'28'}`,boxShadow:isWinner?undefined:`0 0 0 1px rgba(0,0,0,.3),inset 0 0 28px rgba(0,0,0,.25)`}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:9,padding:'13px 12px 9px',flexShrink:0,position:'relative',zIndex:2}}>
        ...
        {isWinner?<div style={{display:'flex',alignItems:'center',gap:3,padding:'3px 8px',borderRadius:20,background:'rgba(245,200,66,.14)',border:'1px solid rgba(245,200,66,.32)'}}><Crown style={{width:10,height:10,color:'#f5c842'}}/><span style={{fontSize:9,fontWeight:800,color:'#f5c842',letterSpacing:'.08em'}}>WIN</span></div>:<Shield style={{width:13,height:13,color:pc,opacity:.3,flexShrink:0}}/>}
      </div>
      ...
      <div style={{padding:'9px 12px 7px',flexShrink:0,position:'relative',zIndex:2}}>
        <div style={{display:'flex',alignItems:'baseline',gap:4}}>
          <motion.span key={total} initial={{y:-6,opacity:0}} animate={{y:0,opacity:1}} transition={{duration:.18}} className="ba-title" style={{fontSize:26,fontWeight:700,color:isWinner?'#f5c842':'#f0eaff',letterSpacing:'.01em',lineHeight:1}}>{total.toLocaleString()}</motion.span>
      ...
      <div style={{padding:'0 10px 10px',flexShrink:0,position:'relative',zIndex:2}}>
        ...
        <div className="ba-spin-slot" style={{borderColor:spinPhase==='gem_spin'?'rgba(167,139,250,.6)':'rgba(157,111,255,.18)',boxShadow:spinPhase==='gem_spin'?'0 0 24px rgba(167,139,250,.35)':undefined}}>
...
const VsDivider=()=>(<div style={{position:'relative',display:'flex',alignItems:'center',justifyContent:'center',padding:'0 3px',flexShrink:0,alignSelf:'stretch'}}><div style={{width:1,height:'100%',background:'rgba(255,255,255,.05)'}}/><div style={{position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-50%)',width:28,height:28,borderRadius:'50%',background:'#07041a',border:'1px solid rgba(255,255,255,.09)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2,boxShadow:'0 0 20px rgba(0,0,0,.9)'}}><span className="ba-vs-badge ba-title" style={{fontSize:10,fontWeight:700,color:'rgba(255,255,255,.3)',display:'block'}}>VS</span></div></div>);
...
const WaitingLobby=({battle,players,teams,userEmail,onJoin,onAddBot,onFillBots,balance})=>{
  ...
  const filled=players.filter(isRealPlayer).length;
...
export default function BattleArena({
  ...
  let payoutLabel='';
  if(done){
    if(isGroup)payoutLabel=`Split: ${Math.floor(totalItemsVal/realPlayerCount).toLocaleString()} coins each`;
      ...
      <div style={{maxWidth:900,margin:'0 auto',display:'flex',flexDirection:'column',gap:14,padding:'0 16px'}}>
        <div style={{position:'relative',overflow:'hidden',borderRadius:16,background:'linear-gradient(120deg,#07041a 0%,#0d0822 50%,#060110 100%)',border:'1px solid rgba(157,111,255,.14)',padding:'14px 18px'}}>
          ...
          <div style={{position:'relative',zIndex:2,display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
            ...
            {blockHash&&(<button onClick={()=>setShowVerifier(true)} style={{display:'flex',alignItems:'center',gap:5,padding:'4px 10px',borderRadius:8,background:'rgba(0,229,160,.08)',border:'1px solid rgba(0,229,160,.22)',color:'#00e5a0',fontSize:10,fontWeight:800,fontFamily:'Outfit,sans-serif',cursor:'pointer',letterSpacing:'.06em',textTransform:'uppercase'}}><Shield style={{width:10,height:10}}/> Provably Fair</button>)}
            <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:5}}>
              {selectedCases.map((_,i)=>(<div key={i} className={i===currentRound&&!done?'ba-pip-live':''} style={{height:5,borderRadius:3,transition:'all .35s',width:i===currentRound?22:i<currentRound?15:7,background:i<currentRound?'#9d6fff':i===currentRound?'#f5c842':'rgba(255,255,255,.1)'}}/>))}
            ...
            <div style={{position:'relative',zIndex:2}}>
              ...
              <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:26,overflowX:'auto',flexWrap:'wrap'}}>
                {teamList.map((mi,ti)=>{
                  ...
                  const teamTotal=mi.reduce((s,pi)=>s+getTotal(pi),0);
                  return(
                    <React.Fragment key={ti}>
                      <motion.div initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{delay:ti*.14}} style={{opacity:isW?1:.22,display:'flex',flexDirection:'column',alignItems:'center',gap:11}}>
                        {teamList.length>1&&<span className="ba-title" style={{fontSize:11,fontWeight:700,color:pal.color,textTransform:'uppercase',letterSpacing:'.12em'}}>Team {ti+1} · {teamTotal.toLocaleString()}</span>}
                        <div style={{display:'flex',gap:16,justifyContent:'center'}}>
                          {mi.filter(pi=>isRealPlayer(rawPlayers[pi])).map(pi=>(
                            <div key={pi} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:7,width:66}}>
                              ...
                              <p className="ba-title" style={{fontSize:17,fontWeight:700,color:isW?'#f5c842':'var(--text-dim)',letterSpacing:'.02em'}}>{getTotal(pi).toLocaleString()}</p>
                            </div>
                          ))}
                        </div>
                      </motion.div>
            ...
            <motion.div key="cd" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0,transition:{duration:.3}}} style={{position:'fixed',inset:0,zIndex:9000,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'rgba(3,0,13,.94)',gap:34,backdropFilter:'blur(8px)'}}>
              ...
              <div style={{display:'flex',gap:22,flexWrap:'wrap',justifyContent:'center',position:'relative',zIndex:2}}>
                {allPIs.map((pi,idx)=>{
                  ...
                  return(<motion.div key={pi} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:idx*.1,type:'spring',stiffness:280,damping:22}} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:9}}><div style={{position:'relative'}}><div style={{position:'absolute',inset:-8,borderRadius:'50%',background:`radial-gradient(circle,${c}1e 0%,transparent 70%)`}}/><PlayerAvatar player={players[pi]} color={c} size={54} iconSize={23}/></div><span style={{fontSize:12,fontWeight:700,color:'rgba(240,234,255,.8)',letterSpacing:'.02em'}}>{players[pi]?.name}</span><div style={{width:30,height:2.5,borderRadius:2,background:c,boxShadow:`0 0 8px ${c}`}}/></motion.div>);
                })}
              </div>
