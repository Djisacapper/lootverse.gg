...
export default function BattleArena({ battle, selectedCases, players: rawPlayers, teams, modeLabel, battleModes = {}, userEmail, onClose, onReward, onJoin, onAddBot, onFillBots, onBattleUpdated, balance = 0 }) {
  ...
  return (
    <div className="space-y-4 relative max-w-full overflow-hidden">
      ...
      {/* ── Battle Over Modal (Portal → true viewport center) ── */}
      {done && (
        <Portal>
          <div
            style={{
          ...
          >
            <motion.div
              ...
              style={{ position: 'relative', margin: 0, left: 'auto', top: 'auto', transform: 'none' }}
              className="rounded-2xl border border-amber-400/30 bg-[#0e0e18] p-6 text-center w-full max-w-xl mx-4 shadow-2xl"
            >
              ...
              <div className="flex items-start justify-center gap-6 overflow-x-auto">
                {teamList.map((mi, ti) => {
                  ...
                  return (
                    <React.Fragment key={ti}>
                        ...
                        <div className="flex gap-3 justify-center flex-wrap">
                          {mi.map(pi => (
                            <div key={pi} className="flex flex-col items-center gap-1">
                              ...
                              <p className="text-xs text-white/60 text-center max-w-[70px] truncate">{players[pi]?.name}</p>
                      ...
                      {ti < teamList.length - 1 && (
                        <div className="flex items-center self-center text-white/20 font-black text-lg flex-shrink-0">VS</div>
                      )}
                    </React.Fragment>
                  );
                })}
