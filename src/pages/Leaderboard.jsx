...
import { Zap, Crown, Flame, Gift, AlertCircle, RefreshCw } from 'lucide-react';
...
export default function Leaderboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Weekly: sort by this week's activity (using same data for now, replace with weekly filter if available)
      const oneWeekAgo = new Date();
      oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);
      setWeeklyUsers(top);

      // Fetch wager data in parallel instead of sequentially
      const wagerResults = await Promise.allSettled(
        top.map(async (u) => {
          try {
            ...
            return { id: u.id, totalWagered };
          } catch {
            return { id: u.id, totalWagered: 0 };
          }
        })
      );
      ...
      wagerResults.forEach((result) => {
        if (result.status === 'fulfilled') {
          wagers[result.value.id] = result.value.totalWagered;
        }
      });
  ...
  const getDisplayName = (u) =>
    u.is_anonymous ? `Anon #${u.id?.slice(-4)}` : u.username || u.full_name || 'Player';

  const getAvatar = (u, size = 'text-3xl') => {
    if (u.avatar_url && u.avatar_url !== 'null') {
      return <img src={u.avatar_url} alt="" className="w-full h-full object-cover" />;
    }
    return <span className={`${size} font-black text-white`}>{u.full_name?.[0]?.toUpperCase() || '?'}</span>;
  };
        ...
        {[1, 0, 2].map((rank) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-sm font-bold text-white text-center max-w-[120px] truncate">{getDisplayName(u)}</p>
              ...
              <div className="text-[10px] text-white/60 mt-1 text-center">
                <p>{wagerLabel}</p>
                <p className={`${wagerColor} font-bold`}>${(userWagers[u.id] || 0).toLocaleString()}</p>
              </div>
  ...
  const ListSection = ({ users, accentColor = 'violet', icon: Icon = Zap, label = 'Top 10', wagerLabel = 'Wagered' }) => (
    <div className="space-y-2">
      ...
      {users.slice(3).map((u, i) => (
        <motion.div
          ...
          className={`glass rounded-xl p-4 flex items-center gap-4 border border-white/5 hover:border-${accentColor}-500/30 transition-colors`}
        >
          <span className={`text-lg font-bold text-${accentColor}-400 w-8 text-center`}>#{i + 4}</span>
          <div className={`w-9 h-9 rounded-full bg-gradient-to-br from-${accentColor}-500/30 to-indigo-500/30 overflow-hidden flex items-center justify-center text-sm font-bold text-white flex-shrink-0`}>
            {getAvatar(u, 'text-sm')}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-white truncate">{getDisplayName(u)}</p>
            <p className="text-[10px] text-white/40">{wagerLabel}: ${(userWagers[u.id] || 0).toLocaleString()}</p>
          </div>
          <div className={`flex items-center gap-1.5 bg-${accentColor}-500/10 rounded-lg px-3 py-1.5 flex-shrink-0`}>
            <Icon className={`w-3.5 h-3.5 text-${accentColor}-400`} />
            <span className={`text-xs font-semibold text-${accentColor}-300`}>Lv {u.level || 1}</span>
          </div>
        </motion.div>
      ))}
    </div>
    ...
    <div className="space-y-8">
      ...
      <Tabs defaultValue="all-time" className="space-y-4">
        ...
        <TabsContent value="all-time" className="space-y-6">
          ...
          <ListSection users={top10} accentColor="violet" icon={Zap} label="Top 10" wagerLabel="Wagered" />
        </TabsContent>

        <TabsContent value="weekly" className="space-y-6">
          <PodiumSection users={weeklyUsers} wagerLabel="This Week" wagerColor="text-amber-300" medals={['🔥', '⚡', '💫']} />
          ...
          <ListSection users={weeklyUsers} accentColor="amber" icon={Flame} label="This Week" wagerLabel="This week" />
        </TabsContent>
      </Tabs>
    </div>
  );
